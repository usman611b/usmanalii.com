...
export class D1ProjectRepository {
  ...
  async updateProject(
    ...
    const sql = `
      UPDATE projects SET
        title = ?, slug = ?, description = ?, detailed_context = ?,
        status = ?, visibility = ?, state = ?, started_at = ?, completed_at = ?,
        role_description = ?, problem_statement = ?, goals = ?, non_goals = ?, constraints = ?,
        contribution_statement = ?, collaboration_context = ?, recruiter_summary = ?,
        deep_dive_content = ?, repository_references = ?, live_demo_references = ?,
        hero_artifact_id = ?, case_study_body = ?, scheduled_for = ?, embargo_until = ?,
        is_featured = ?,
        updated_at = ?,
        version_no = ?
    ...
    const res = await this.db
      ...
      .bind(
        ...
        next.endDate || null,
        next.role || null,
        next.problemStatement || null,
        JSON.stringify(next.goals || []),
        JSON.stringify(next.nonGoals || []),
        JSON.stringify(next.constraints || []),
        next.contributionStatement || null,
        next.collaborationContext || null,
        next.recruiterSummary || null,
        next.deepDiveContent || null,
        JSON.stringify(next.repositoryReferences || []),
        JSON.stringify(next.liveDemoReferences || []),
        next.heroArtifactId || null,
        next.caseStudyBody || null,
        next.scheduledFor || null,
        next.embargoUntil || null,
        next.isFeatured ? 1 : 0,
        now,
        newVersion,
        ownerId,

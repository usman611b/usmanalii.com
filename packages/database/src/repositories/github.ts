...
import type {
  ...
  GitHubImportedObjectEntity,
  GitHubOwnerIdentityEntity,
  GitHubRateLimitEntity,
...
export class D1GitHubRepository {
  ...
  async toggleRepositorySync(
    ...
    const res = await this.db.prepare(sql).bind(selectedForSync ? 1 : 0, new Date().toISOString(), ownerId, id).run();
  ...
  async linkRepositoryToProject(
    ...
    const res = await this.db.prepare(sql).bind(projectId, new Date().toISOString(), ownerId, repositoryId).run();
    return (res.meta.rows_written ?? 0) > 0;
  }
  ...
  async getCheckpoint(
    ...
    const row = await this.db.prepare(sql).bind(ownerId, repositoryId, resourceType).first<Record<string, unknown>>();
    if (!row) return null;
    return {
  ...
  async upsertCheckpoint(
    ...
    const sql = `
      ...
      ON CONFLICT(owner_id, repository_id, resource_type) DO UPDATE SET
        ...
        updated_at = excluded.updated_at
    `;
    await this.db.prepare(sql).bind(id, ownerId, repositoryId, resourceType, checkpoint.cursor ?? null, checkpoint.etag ?? null, checkpoint.lastModified ?? null, now).run();
  }
  ...
  async upsertImportedObjects(
    ...
    const stmts = objects.map((obj) => {
      ...
      return this.db.prepare(sql).bind(
        ...
        obj.contentHash,
        typeof obj.rawPayloadSanitized === 'string' ? obj.rawPayloadSanitized : JSON.stringify(obj.rawPayloadSanitized),
  ...
  async getCandidateById(
    ownerId: string,
    id: string,
  ): Promise<EvidenceCandidateEntity | null> {
  ...
  async acceptCandidate(
    ...
    const batchStmts = [
      this.db.prepare(insertEvidenceSql).bind(
        evidenceItemId,
        ownerId,
      ...
      this.db.prepare(updateCandidateSql).bind(
    ...
    if (options.linkProjectId) {
      ...
      `;
      batchStmts.push(
        this.db.prepare(insertLinkSql).bind(
          linkId,
          evidenceItemId,
          options.linkProjectId,
          ownerId,
          now,
          now,
          now,
  ...
  async rejectCandidate(
    ...
    const res = await this.db.prepare(sql).bind(reason, new Date().toISOString(), ownerId, candidateId).run();
    return (res.meta.rows_written ?? 0) > 0;
  }
  ...
  async recordRateLimitSnapshot(
    ownerId: string,
    snapshot: { limitTotal: number; remaining: number; resetAt: string; used: number; resourceCategory?: string },
  ): Promise<void> {
    ...
    const sql = `
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `;
    await this.db.prepare(sql).bind(id, ownerId, snapshot.limitTotal, snapshot.remaining, snapshot.resetAt, snapshot.used, snapshot.resourceCategory ?? 'core', now).run();
  }
  ...
  private mapCandidateRow(row: Record<string, unknown>): EvidenceCandidateEntity {
    return {
      ...
      acceptedEvidenceItemId: row.accepted_evidence_item_id ? String(row.accepted_evidence_item_id) as any : null,
      createdAt: String(row.created_at) as any,
      updatedAt: String(row.updated_at) as any,
    };
  }
